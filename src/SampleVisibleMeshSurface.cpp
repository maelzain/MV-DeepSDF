...
void SampleFromSurfaceInside(
  ...
  const pangolin::Image<float> vertices =
  ...
  for (const auto& object : geom.objects) {
    ...
    if (it_vert_indices != object.second.attributes.end()) {
      ...
      for (uint i = 0; i < modelFaces.h; ++i) {
        linearized_faces.emplace_back(modelFaces(0, i), modelFaces(1, i), modelFaces(2, i));
      }
    }
  }
  ...
  for (const Eigen::Vector3i& face : linearized_faces) {
    float area = TriangleArea(
        (Eigen::Vector3f)Eigen::Map<Eigen::Vector3f>(const_cast<float*>(vertices.RowPtr(face(0)))),
        (Eigen::Vector3f)Eigen::Map<Eigen::Vector3f>(const_cast<float*>(vertices.RowPtr(face(1)))),
        (Eigen::Vector3f)Eigen::Map<Eigen::Vector3f>(const_cast<float*>(vertices.RowPtr(face(2)))));
  ...
  while ((int)surfpts.size() < num_sample) {
    ...
    Eigen::Vector3f point = SamplePointFromTriangle(
        Eigen::Map<Eigen::Vector3f>(const_cast<float*>(vertices.RowPtr(face(0)))),
        Eigen::Map<Eigen::Vector3f>(const_cast<float*>(vertices.RowPtr(face(1)))),
        Eigen::Map<Eigen::Vector3f>(const_cast<float*>(vertices.RowPtr(face(2)))));
...
int main(int argc, char** argv) {
  ...
  // linearize the object indices
  {
    ...
    for (const auto& object : geom.objects) {
      ...
      if (it_vert_indices != object.second.attributes.end()) {
        pangolin::Image<uint32_t> modelFaces =
            std::get<pangolin::Image<uint32_t>>(it_vert_indices->second);
        for (int i = 0; i < modelFaces.h; ++i) {
          new_ibo.Row(index).CopyFrom(modelFaces.Row(i));
          ++index;
        }
      }
    }
  ...
  for (unsigned int v = 0; v < views.size(); v++) {
    ...
    prog.SetUniform("ttt", 1.0f, 0.0f, 0.0f, 1.0f);
